...
struct hmacalg {
	...
	uint8_t size_block;    /* HMAC split key size */
};
...
struct hmacctx {
	...
	const struct hmacalg *alg;            /* Reference to the algo constants */
};

struct crypto_hmac {
	struct crypto_mac_ctx hmac_ctx; /* Crypto Hash API context */
	struct hmacctx *ctx;             /* Hash Context */
};

#define OP_ALG(alg)	((SECURE_SERVICE_HASH_##alg) & 0xF)

static const struct hmacalg hmac_alg[] = {
	{
		/* md5 */
		.type = OP_ALG(MD5),
...
static struct crypto_hmac *to_hmac_ctx(struct crypto_mac_ctx *ctx)
{
	...
	return container_of(ctx, struct crypto_hmac, hmac_ctx);
}
			   ...
			   size_t len)
{
	...
	hash->ctx->cmd_desc.hash.cmd_cfg.mode = SECURE_SERVICE_HMAC_MODE;
	hash->ctx->cmd_desc.hash.cmd_cfg.in_ctrl = 0xF;
	hash->ctx->free_len = CACHED_DATA_LEN;

	/* key len is greater than block size, make a hash for key */
	if (len  > hash->ctx->alg->size_block) {
		uint8_t *aligned_key_buf = (uint8_t *)key;

		if ((size_t)key & 0x3) {
			aligned_key_buf = memalign(4, len);
			if (aligned_key_buf == NULL) {
				EMSG("alloc key buffer fail\n");
				return TEE_ERROR_OUT_OF_MEMORY;
			}
			memcpy(aligned_key_buf, key, len);
		}
		hash->ctx->cmd_desc.hash.cmd_cfg.mode = SECURE_SERVICE_HASH_MODE;
		hash->ctx->cmd_desc.hash.cmd_cfg.in_ctrl = SECURE_SERVICE_IN_ALL;
		nuclei_hsm_calc_hash(aligned_key_buf, len, hash->ctx->cmd_desc.hash.cmd_cfg,
			hash->ctx->cmd_desc.key, hash->ctx->alg->size_digest);
		hash->ctx->cmd_desc.hash.cmd_cfg.keyLen = hash->ctx->alg->size_digest;
		hash->ctx->cmd_desc.hash.cmd_cfg.mode = SECURE_SERVICE_HMAC_MODE;
		hash->ctx->cmd_desc.hash.cmd_cfg.in_ctrl = 0xF;
	} else {
		memcpy(hash->ctx->cmd_desc.key, key, len);
		hash->ctx->cmd_desc.hash.cmd_cfg.keyLen = len;
	}
...
{
	struct crypto_hmac *hash_src = to_hmac_ctx(src_ctx);
	struct crypto_hmac *hash_dst = to_hmac_ctx(dst_ctx);
...
static const struct hmacalg *nuclei_hmac_get_alg(uint32_t algo)
{
	uint8_t hash_id = TEE_ALG_GET_MAIN_ALG(algo);
	unsigned int idx = hash_id - TEE_MAIN_ALGO_MD5;

	if (idx > ARRAY_SIZE(hmac_alg))
		return NULL;
 ...
 * @algo   Algorithm ID
 */
static TEE_Result nuclei_hmac_allocate(struct crypto_mac_ctx **ctx,
				     uint32_t algo)
{
	struct crypto_hmac *hash = NULL;
	struct hmacctx *hmac_ctx = NULL;
	const struct hmacalg *alg = NULL;
	...
	*ctx = NULL;
	...
	hmac_ctx = memalign(4, sizeof(*hmac_ctx));
	if (!hmac_ctx) {
	...
	hmac_ctx->alg = alg;
	hash->hmac_ctx.ops = &hmac_ops;
	hash->ctx = hmac_ctx;

	*ctx = &hash->hmac_ctx;

	DMSG("Allocated Context (%p)", hmac_ctx);
...
{
	return drvcrypt_register_hmac(&nuclei_hmac_allocate);
}